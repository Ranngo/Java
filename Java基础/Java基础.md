...
* 所有异常分为Error和Exception，均由Throwable继承而来

* Error：指计算机运行时的系统错误或者内存等资源耗尽问题

* Exception：

  * RuntimeException：指常见的可避免的程序编写错误导致的异常，通常包括以下几种：
    * 错误的类型转换
    * 数组指针越界
    * 访问空指针

  * 其他异常：指I/O错误等问题导致的异常，常见类型有：
    * 试图在文件尾部后面读取数据
    * 试图打开不存在的文件
    * 试图根据给定的字符串查找类对象，但该字符串表示的类并不存在

通常将上述三种异常分为：未检查(unchecked)异常和已检查(checked)异常

* 未检查异常：是指继承于Error或者RuntimeException的异常不需要进行声明抛出异常
* 已检查异常：指其他异常类型，这类异常需要在方法首部声明所有可能抛出的异常
